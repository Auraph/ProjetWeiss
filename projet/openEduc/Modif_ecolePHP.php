...
if(isset($_SESSION['droit'])){
    if($_SESSION['idEcoleUser']==$_SESSION['id'] || $_SESSION['droit']==2 || $_SESSION['droit']==3){
...
function mailC($val,$errormsg){
    ...
    if (filter_var($val,FILTER_VALIDATE_EMAIL)) {
        ...
        header("location:Modif_ecoleHTML.php?id=".$_session['id']);
        exit();
    }
} 
}
...
$sql1 = "REPLACE INTO adresse(id,numero, rue, commune, cp) VALUES (?,?,?,?,?)";
$res = $conn->prepare($sql1);
$exec = $res->execute(array($rest['idAdresse'],$numRue, $rue, $com, $cp));

echo $idAdresse = $conn->lastInsertId();


$sql2 = "REPLACE INTO ecole(id,nom, identifiant, idAdresse, telephone, mail, dateMiseAJour, anneeScolaire) VALUES (?,?,?,?,?,?,?,?) ";
$res = $conn->prepare($sql2);
$exec = $res->execute(array($_SESSION['id'],$nomEcole, $identifiantEcole, $idAdresse, $phoneEcole, $mailE, $datemaj, $annsco));


$sqls="DELETE FROM classe WHERE idEcole=?";
$ress=$conn->prepare($sqls);
$ress->execute([$_SESSION['id']]);
...
$sqls="DELETE FROM correspondant WHERE idEcole=?";
$ress=$conn->prepare($sqls);
$ress->execute([$_SESSION['id']]);
...
header("location:Ecole.php?id=".$idEcole);
